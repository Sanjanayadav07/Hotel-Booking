...
const AllRooms = () => {
    ...
    //func to check if a  room matches selected room type

    const matchesRoomType = (room) => {
        return selectedFilters.roomTypes.length === 0 || selectedFilters.roomTypes.includes(room.roomTypes);
    }

    //function to check if a room matchesis selected price length
    ...
    //fiter and sort rooms based on selected filters and sort options
        ...
        <div className='flex flex-col-reverse lg:flex-row items-start justify-between pt-28 md:pt-35 px-4 md:px-16 lg:px-24 xl:px-32 '>
            <div className=''>
                ...
                {filteredRooms.map((room) => (
                    <div
                        key={room._id}
                        className="flex flex-col md:flex-row items-center gap-6 py-10 border-b border-gray-300 last:pb-30 last:border-0"
                    >
                        <img
                            ...
                            className="max-h-56 md:w-1/2 rounded-xl shadow-lg object-cover cursor-pointer hover:opacity-90 transition"
                        />

                        <div className="md:w-1/2 flex flex-col justify-center gap-2">
                            <p className="text-gray-500">{room.hotel.city}</p>
                            <p
                                className="text-gray-800 text-2xl md:text-3xl font-playfair cursor-pointer hover:text-blue-600 transition"
